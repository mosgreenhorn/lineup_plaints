...
function deriveType(label, value, column, data, options) {
    ...
    if (typeof value === 'number') {
        base.type = 'number';
        base.domain = extent(data, function (d) { return d[column]; });
        return base;
    }
    ...
    if (typeof value === 'string') {
        var categories = new Set(data.map(function (d) { return d[column]; }));
    ...
    if (Array.isArray(value)) {
        ...
        if (typeof vs === 'number') {
            base.type = 'numbers';
            base.domain = extent((_a = []).concat.apply(_a, data.map(function (d) { return d[column]; })));
            return base;
        }
        ...
        if (typeof value === 'string') {
            var categories = new Set((_b = []).concat.apply(_b, data.map(function (d) { return d[column]; })));
            if (categories.size < data.length * options.categoricalThreshold) {
...
export function deriveColumnDescriptions(data, options) {
    ...
    return columns.map(function (key) { return deriveType(key, first[key], key, data, config); });
}
