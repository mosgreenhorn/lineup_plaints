...
import { isSortAble, isGroupAble, getToolbarDialogAddons, dialogContext, isGroupSortAble } from '../toolbar';
...
var Hierarchy = (function () {
    ...
    Hierarchy.prototype.addAdder = function (adder, ranking, addonKey, current, node, check, onSelect) {
        ...
        adder.on(SearchBox.EVENT_SELECT, function (item) {
            ...
            if (addons.length > 0) {
                _this.customize(item.col, addons, { currentTarget: adder.node }, function () { return onSelect(item.col); });
            }
            else {
                onSelect(item.col);
            }
    ...
    Hierarchy.prototype.customize = function (col, addons, evt, onClick) {
        var dialog = new AddonDialog(col, addons, dialogContext(this.ctx, 0, evt), this.ctx, onClick);
        dialog.open();
    };
    return Hierarchy;
}());
